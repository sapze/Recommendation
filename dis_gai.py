...
class DIS():
    def __init__(self, itemNum, userNum, relationNum, emb_dim, lamda, param=None, initdelta=0.05, learning_rate=0.05):
        ...
        self.d_params = [self.user_embeddings, self.item_coll_embeddings, self.item_struct_embeddings, self.r_embeddings, self.item_bias]
        ...
        self.pre_loss = tf.nn.sigmoid_cross_entropy_with_logits(labels=self.label,
                                                                logits=self.pre_logits) +  tf.reduce_sum(abs(self.i_struct_embedding + self.r_embedding - self.i_struct_embedding), 1, keepdims=True) + self.lamda * (
            tf.nn.l2_loss(self.u_embedding) + tf.nn.l2_loss(self.i_embedding) + tf.nn.l2_loss(self.i_bias)
        )
