...
class GEN():
    def __init__(self, itemNum, userNum, relationNum, emb_dim, lamda, param=None, initdelta=0.05, learning_rate=0.05):
        ...
        with tf.variable_scope('generator'):
            if self.param == None:
                self.user_embeddings = tf.Variable(
                    tf.random_uniform([self.userNum, self.emb_dim], minval=-self.initdelta, maxval=self.initdelta,
                                      dtype=tf.float32))
                self.item_coll_embeddings = tf.Variable(
                    tf.random_uniform([self.itemNum, self.emb_dim], minval=-self.initdelta, maxval=self.initdelta,
                                      dtype=tf.float32))
                # myself
                self.item_struct_embeddings =  tf.Variable(
                    tf.random_uniform([self.itemNum, self.emb_dim], minval=-self.initdelta, maxval=self.initdelta,
                                      dtype=tf.float32))
                self.r_embeddings =  tf.Variable(
                    tf.random_uniform([self.relationNum, self.emb_dim], minval=-self.initdelta, maxval=self.initdelta,
                                      dtype=tf.float32))
            ...
            self.g_params = [self.user_embeddings, self.item_coll_embeddings, self.item_struct_embeddings, self.r_embeddings, self.item_bias]

        self.u = tf.placeholder(tf.int32)
        self.i = tf.placeholder(tf.int32)
        self.r = tf.placeholder(tf.int32)
        self.reward = tf.placeholder(tf.float32)
        
        # myself
        self.item_embeddings = self.item_coll_embeddings + self.item_struct_embeddings
        #self.item_embeddings = tf.concat([self.item_coll_embeddings, self.item_struct_embeddings], 1)

        self.u_embedding = tf.nn.embedding_lookup(self.user_embeddings, self.u)
        self.i_embedding = tf.nn.embedding_lookup(self.item_embeddings, self.i)
        self.i_bias = tf.gather(self.item_bias, self.i)
        
        self.i_struct_embedding = tf.nn.embedding_lookup(self.item_embeddings, self.i)
        self.r_embedding = tf.nn.embedding_lookup(self.r_embeddings, self.r)
        ...
        self.gan_loss = -tf.reduce_mean(tf.log(self.i_prob) * self.reward) +  tf.reduce_sum(abs(self.i_struct_embedding + self.r_embedding - self.i_struct_embedding), 1, keepdims=True) + self.lamda * (
            tf.nn.l2_loss(self.u_embedding) + tf.nn.l2_loss(self.i_embedding) + tf.nn.l2_loss(self.i_bias)
        )
